...
def augmentation(data,target):
    # Randomly choose one of the augmentation methods
    aug_data = []
    augmentation_method = [no_augmentation, axis_shifting, random_masking, jittering]
    # Apply the chosen augmentation method to the sequence
    for aug in augmentation_method:
        if aug == axis_shifting:
            augmented_sequence, augmented_target = aug(data,target)
        else:
            augmented_sequence, augmented_target = aug(data,target)
            
        aug_data.append(np.squeeze(augmented_sequence.astype(np.float32), -1))
...
class Fish(Dataset):       
    ...
    def __getitem__(self, index):
        ...
        if self.mode=='original':
            ...
            out_dim,out_label = augmentation(out_dim,out_label)
    ...
    def __len__(self):
        ...
        return int(round(len(self.data)*0.01))
